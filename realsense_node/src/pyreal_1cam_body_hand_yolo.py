...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        # hand
        left_hand_pixel, left_hand = keypoint_to_xyz(closest_keypoints,9,depth_image_flipped,rs,intr,depth_scale)
        (left_hand_pixel_x, left_hand_pixel_y),(left_hand_x, left_hand_y, left_hand_z) = left_hand_pixel, left_hand

        right_hand_pixel, right_hand = keypoint_to_xyz(closest_keypoints,10,depth_image_flipped,rs,intr,depth_scale)
        (right_hand_pixel_x, right_hand_pixel_y),(right_hand_x, right_hand_y, right_hand_z) = right_hand_pixel, right_hand


        realsense_p.hands_msg.data = np.array([left_hand_x, left_hand_y, left_hand_z,
        ...
        if RENDER == True:
            ...
            cv2.circle(images, (leftshoulder_pixel_x, leftshoulder_pixel_y), 5, (0, 255, 0), -1) # green circle
            cv2.circle(images, (leftelbow_pixel_x, leftelbow_pixel_y), 5, (0, 255, 0), -1) # green circle
            cv2.circle(images, (rightshoulder_pixel_x, rightshoulder_pixel_y), 5, (0, 255, 0), -1) # green circle
            cv2.circle(images, (rightelbow_pixel_x, rightelbow_pixel_y), 5, (0, 255, 0), -1) # green circle

            cv2.circle(images, (left_hand_pixel_x, left_hand_pixel_y), 5, (0, 0, 255), -1) # red circle
            cv2.circle(images, (right_hand_pixel_x, right_hand_pixel_y), 5, (0, 0, 255), -1) # red circle
